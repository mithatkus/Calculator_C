...
/*
  ...
  code.  However, while we gain certain advantages it is worth also
...
/* We must enclose the file name in either double quotes "" or angle
   ...
   On the other hand if we use angle brackets as below you the
...
// Declarations of the stardard input/ouput libc functions and types
...
/* Header files
 ...
 not in the header file.  Rather there are seperate source files that
 contain the actual definitions of the functions.

 One of the most powerfull aspects of the C toolchain model is the
 ...
 Given that we have a seperate csum.c that when compiled and assemble
...
/* Unions -- multiple views of a chunck of memory
...
/* Structures:  heterogeneous complicated views of memory
   ...
   contraints that our particular computer might have.  The 'sizeof()'
...
// Discussion of these are provided with their defintions after the
// main function
...
/* main: the first application programmer function to be called
  ...
  the C startup code.  The C runtime passes two standard paramters to
...
main(int argc, char **argv, char *envp)
{
  ...
  for (cmd = &CALC_DATA_BEGIN;  // initalize the cmd varible with the address
    ...
    case '&':
      ...
      // Unlike our assembly version we C does not let us explicity
...
// Another powerfull aspect of C is the ability to work with text
...
{
  // print to standard error the current values as line ascii line
  ...
  // use fprintf to print SUM_POSITIVE and SUM_NEGATIVE finish
  // printing the line note space seperate the values and put a new
  // line at the end.
  // HINT: the format string for a long long decimal is "%lld" and a newline is "\n"
  // force crash -- replace with your code here
  fprintf(stderr,"%lld ", SUM_POSITIVE);
  fprintf(stderr,"%lld\n", SUM_NEGATIVE);
...
writeResults(long long result) {
  ...
  long long len_data = ((void *)&CALC_DATA_END) - ((void *)&CALC_DATA_BEGIN);
  if (write(1,(void *)&CALC_DATA_BEGIN, len_data) != len_data) {
    fprintf(stderr, "ERROR: write number of bytes failed\n");
  }
