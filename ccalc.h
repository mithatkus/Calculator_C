...
/* HEADER Files: This is an example of a header file.  We use it to
  declare all the things that a c file might need to work with rest of
  the calculator code.  In general we avoid putting any definitions in
...
// declare the externals for SUM_POSITIVE and SUM_POSITIVE the use of
...
/* Functon pointers
   ...
   allow us to introduce our own custom name for a type.  Eg
    ...
    Eg to define a pointer variable 'func_ptr' that shoud be treated
...
/*
  ...
  place where VPRINT appears in the source will be subsituted with a
  ...
  error however you can also can also set the compilation to delete
